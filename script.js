document.addEventListener('DOMContentLoaded', () => {
    // Existing elements and functionality
    ...
    const displayHours = () => {
        ...
        hoursList.innerHTML = hours.map(hour => `<li>${hour.student}: ${hour.amount} hours</li>`).join('');
    };
    ...
    quizForm.addEventListener('submit', (e) => {
        ...
        if (totalPoints <= 7) {
            ...
            finalResult = 'You are an Artist! You are creative and express yourself through art. Therefore, we recommend volunteering in places that allow your creative juices to flow such as providing artistic works or teaching art.';
        }
        interestsDiv.textContent = `Interests: ${finalResult}`;
        localStorage.setItem('interests', finalResult);
    });
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        // For demonstration, we'll just show the organization dashboard
        quizSection.style.display = 'none';
        loginSection.style.display = 'none';
        orgDashboard.style.display = 'block';
    });

    // Registration button event listeners
    ...
    studentRegisterForm.addEventListener('submit', (e) => {
        ...
        // Save the student info to local storage
